...
import cn.zwz.doctor.entity.S;
import cn.zwz.doctor.mapper.SMapper;
...
public class HospitalOrderController {
    ...
    public Result<Object> payMoney(@RequestParam String orderId){
        ...
        if(ho.getStatus()==1){
            return ResultUtil.error("你不能对已经被取消的订单付款");
        }
        if(ho == null) {
    ...
    public Result<Object> addOrder(@RequestParam String orderId){
        ...
        queryWrapper.eq("name",ds.getDoctorName());
        S s = mapper.selectOne(queryWrapper);
        //MB条件修改
        UpdateWrapper<S> updateWrapper = new UpdateWrapper<>();
        //where
        updateWrapper.eq("name",s.getName());
        //set
        int newone = s.getWages()+1;
        updateWrapper.set("wages",newone);
